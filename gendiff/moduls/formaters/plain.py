    ...
    for i in diff:
        if not type(diff.get(i)) == dict:
            ...
            if i.startswith('del.'):
                text += f"Property '{'.'.join(depth + [i[4:]])}'" \
                        f" was removed\n"
            if i.startswith('ch-.') and \
                    type(diff.get(i.replace('ch-.', 'ch+.'))) != dict:
                text += f"Property '{'.'.join(depth + [i[4:]])}'" \
        ...
        if type(diff.get(i)) == dict:
            ...
            if i.startswith('ch-.'):
                text += f"Property '{'.'.join(depth + [i[4:]])}'" \
                        f" was updated. From [complex value] to " \
                        f"'{diff.get(i.replace('ch-.', 'ch+.'))}'\n"
            if i.startswith('ch+.'):
                text += f"Property '{'.'.join(depth + [i[4:]])}' was updated." \
                        f" From '{diff.get(i.replace('ch+.', 'ch-.'))}' to " \
                        f"[complex value]\n"
            if not i.startswith('del.') and not i.startswith('add.') and \
                    not i.startswith('ch-.') and not i.startswith('ch+.'):
                children = diff.get(i)
